...
from daily_flow.db.errors import DBIntegrityError, ParentNotFoundError, RepoError
...
from daily_flow.services.errors import TemporaryError, UserInputError
...
class ActivityCategoryService:
    ...
    async def assign_category_to_activity(self, dto: CategoryToActivityDTO) -> bool:
        category_id, activity_id = dto.category_id, dto.activity_id
        try:
            return await self._repo.assign_category_to_activity(
                category_id=category_id, activity_id=activity_id
            )
        except ParentNotFoundError as e:
            raise UserInputError(str(e)) from e
        except (DBIntegrityError, RepoError) as e:
            logger.exception(
                "ActivityCategoryService.assign_category_to_activity failed "
    ...
    async def delete_category_from_activity(self, dto: CategoryToActivityDTO) -> int:
        category_id, activity_id = dto.category_id, dto.activity_id
        try:
            return await self._repo.delete_category_from_activity(
                category_id=category_id, activity_id=activity_id
            )
        except ParentNotFoundError as e:
            raise UserInputError(str(e)) from e
    ...
    async def get_categories_by_activity(self, activity_id: int) -> list[Category]:
        ...
        try:
            return await self._repo.get_categories_by_activity(activity_id)
        except (RepoError, SQLAlchemyError) as e:
            raise TemporaryError("Database error. Please try again.") from e
    ...
    async def get_activities_by_category(self, category_id: int) -> list[Activity]:
        ...
        try:
            return await self._repo.get_activities_by_category(category_id)
        except (RepoError, SQLAlchemyError) as e:
            raise TemporaryError("Database error. Please try again.") from e
