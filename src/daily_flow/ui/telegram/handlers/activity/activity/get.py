...
from daily_flow.ui.telegram.render.activity import render_activity, render_activity_compact
from daily_flow.ui.telegram.runtime import router
from daily_flow.ui.telegram.states import ActivityByCategoryGetForm, ActivityGetForm
...
async def get_all_activities(message: types.Message, db_container: Container):
    try:
        ...
        await message.answer(
            "❌ Сталася помилка під час отримання списку активностей.",
...
async def ask_activity_ref(message: types.Message, state: FSMContext, db_container: Container):
    ...
    await message.answer(
        f"Введи **ID** або **назву (title)** активності.\n\n{activities_text}",
        reply_markup=ActivityMenu.get(),
        parse_mode="Markdown",
    )
    ...
    try:
        ...
        if not activity:
            return await message.answer(
                "🔍 Не знайшов активність за цим значенням.", reply_markup=ActivityMenu.get()
            )

        await message.answer(
            render_activity(activity), reply_markup=ActivityMenu.get(), parse_mode="Markdown"
        )
        ...
        await state.clear()
        await message.answer(
            "❌ Сталася помилка під час пошуку активності.", reply_markup=ActivityMenu.get()
        )


@router.message(F.text == ActivityMenu.BTN_GET_ACTIVITIES_BY_CATEGORY)
async def ask_category_for_activities(
    ...
    await message.answer(
        f"Введи **ID категорії**, щоб показати активності в ній.\n\n{categories_text}",
        reply_markup=ActivityMenu.get(),
        parse_mode="Markdown",
    )
...
async def perform_get_activities_by_category(
    ...
    try:
        ...
        if not activities:
            return await message.answer(
                f"📌 За категорією ID={category_id} активностей поки немає.",
                reply_markup=ActivityMenu.get(),
            )
        ...
        await state.clear()
        await message.answer(
            "❌ Сталася помилка під час отримання активностей за категорією.",
            reply_markup=ActivityMenu.get(),
        )
