...
async def edit_any_activity_category_field(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"Введіть нові дані про {activity_category_mapping.get(field_name)}:"
    )
...
async def process_activity_category_input(message: types.Message, state: FSMContext, bot: Bot):
    ...
    await refresh_form_message(
        ...
        bot=bot,
        text=text,
        form_name=ACTIVITY_CATEGORY_FORM,
...
async def render_upsert_activity_category(state: FSMContext) -> str:
    ...
    text += "\n".join(
        f"{activity_category_mapping.get(k).capitalize()}: {val(k)}"
        for k in activity_category_mapping.keys()
    )
    return text
