...
async def delete_category_from_activity(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "Введи **Activity ID**, з якого треба прибрати категорію:",
...
async def delete_category_from_activity_step1(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "Тепер введи **Category ID**, яку треба прибрати:",
        reply_markup=ActivityMenu.get(),
        parse_mode="Markdown",
    )
...
async def delete_category_from_activity_step2(
    ...
    try:
        dto = CategoryToActivityDTO(category_id=category_id, activity_id=activity_id)

        deleted = await db_container.activity_category_service.delete_category_from_activity(
            dto
        )  # process_dto не використовуємо у delete
        await state.clear()
        ...
        if deleted > 0:
            ...
            await message.answer(
                "🔍 Нічого не видалено (звʼязок не знайдено).", reply_markup=ActivityMenu.get()
            )
        ...
        await state.clear()
        await message.answer(
            "❌ Сталася помилка під час видалення звʼязку.", reply_markup=ActivityMenu.get()
        )
