...
async def edit_any_mood_log_field(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"Введіть ваші нові дані про {mood_log_mapping.get(field_name)}:"
    )
    await state.set_state(MoodLogForm.waiting_input)


@router.message(MoodLogForm.waiting_input)
async def process_mood_input(message: types.Message, state: FSMContext, bot: Bot):
        ...
        try:
            ...
            if not 1 <= mood <= 4:
                return await message.answer(
                    "Будь ласка, введіть число від 1 до 4 (де 1 — мінімум, 4 — максимум)."
                )
    ...
    await refresh_form_message(
        text=text, state=state, bot=bot, form_name=MOOD_LOG_FORM, mapping=mood_log_mapping
    )
...
async def show_mood_upsert_data(message: types.Message, state: FSMContext):
    ...
    sent_message = await message.answer(
        text, reply_markup=get_form_keyboard(mood_log_mapping, MOOD_LOG_FORM), parse_mode="Markdown"
    )
