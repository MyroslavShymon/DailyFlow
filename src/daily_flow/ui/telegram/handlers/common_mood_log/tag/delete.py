...
async def perform_tag_impact_delete(
    ...
    try:
        ...
        deleted_count = await db_container.common_mood_log_service.delete_tag_by_day(
            day=selected_date, tag=tag
        )

        if deleted_count > 0:
            await state.clear()
            await message.answer(
                "✅ Дані видалено! Повертаємось у меню настрою:", reply_markup=CommonMoodMenu.get()
            )
        elif deleted_count == 0:
            await message.answer(
                f"🔍 Схоже, з чинником {tag} ще немає жодного запису "
                f"або видалення не відбулось з інших причин",
                reply_markup=CommonMoodMenu.get(),
            )
    except ValueError:
...
async def delete_tag_impact_step1(message: types.Message, state: FSMContext):
...
async def delete_tag_impact_step2(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        ...
        reply_markup=get_date_keyboard(
            DateAction.DELETE, "tag_impact", optional_fields={"tag": tag_to_delete}
        ),
        parse_mode="Markdown",
    )
