...
def get_date_keyboard(action: DateAction, entity: str, optional_fields=None):
    ...
    builder.button(
        text=f"Сьогодні ({today.strftime('%d.%m')})", callback_data=mk(today.strftime("%d-%m-%Y"))
    )
    builder.button(
        text=f"Вчора ({yesterday.strftime('%d.%m')})",
        callback_data=mk(yesterday.strftime("%d-%m-%Y")),
    )
    builder.button(text="Ввести вручну ✍️", callback_data=mk("manual"))
