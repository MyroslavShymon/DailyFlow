...
from daily_flow.db.schema import idea, idea_sphere, sphere
...
class IdeaRepo:
    ...
    async def upsert_sphere(self, name: str, description: str | None) -> Sphere:
        name = (name or "").strip()
        if not name:
        ...
        if description is None:
            stmt = insert_stmt.on_conflict_do_nothing(index_elements=[sphere.c.name]).returning(
                *sphere.c
            )
        else:
            stmt = insert_stmt.on_conflict_do_update(
                index_elements=[sphere.c.name], set_={"description": description}
            ).returning(*sphere.c)
        ...
        try:
            async with self._engine.begin() as conn:
                ...
                if row is None:
                    res = await conn.execute(select(*sphere.c).where(sphere.c.name == name))
                    row = res.mappings().one()
                return self._to_sphere(row)
        except IntegrityError as e:
            logger.exception(
                "IdeaRepo.upsert_sphere failed (name=%s, description=%s)",
                name,
                description,
            )
            raise map_integrity_error(e, sphere.name) from e

    async def upsert_idea(self, title: str, payload: dict[str, Any]) -> Idea:
        ...
        insert_stmt = sqlite_insert(idea).values(title=title, **payload)

        fields_to_update = {k: v for k, v in payload.items() if v is not None}
        if not fields_to_update:
            stmt = insert_stmt.on_conflict_do_nothing(index_elements=[idea.c.title]).returning(
                *idea.c
            )
        else:
            stmt = insert_stmt.on_conflict_do_update(
                index_elements=[idea.c.title], set_=fields_to_update
            ).returning(*idea.c)
        ...
        try:
            async with self._engine.begin() as conn:
                ...
                row = res.mappings().one_or_none()
                if row is None:
                    res = await conn.execute(select(*idea.c).where(idea.c.title == title))
                    row = res.mappings().one()
                return self._to_idea(row)
        except IntegrityError as e:
            logger.exception(
                "IdeaRepo.upsert_idea failed (title=%s, fields=%s)",
                title,
                list(payload.keys()),
            )
            raise map_integrity_error(e, idea.name) from e

    async def assign_sphere_to_idea(self, sphere_id: int, idea_id: int) -> bool:
        try:
            async with self._engine.begin() as conn:
            ...
            logger.exception(
                "IdeaRepo.assign_sphere_to_idea failed (sphere_id=%s, idea_id=%s)",
                sphere_id,
                idea_id,
            )
    ...
    async def delete_idea_by_title(self, title: str) -> int:
        stmt = delete(idea).where(idea.c.title == title)
    ...
    async def delete_sphere_from_idea(self, idea_id: int, sphere_id: int) -> int:
        async with self._engine.begin() as conn:
            await self._assert_exists(conn, idea, idea_id, "idea")
            await self._assert_exists(conn, sphere, sphere_id, "sphere")
            stmt = delete(idea_sphere).where(
                and_(idea_sphere.c.idea_id == idea_id, idea_sphere.c.sphere_id == sphere_id)
            )
