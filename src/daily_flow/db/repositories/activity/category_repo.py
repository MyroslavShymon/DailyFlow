...
from daily_flow.db.errors import MissingRequiredFieldError, map_integrity_error
...
class CategoryRepo:
    ...
    async def upsert_category(self, name: str, description: str | None) -> Category:
        ...
        if description is None:
            stmt = insert_stmt.on_conflict_do_nothing(index_elements=[category.c.name]).returning(
                *category.c
            )
        else:
        ...
        try:
            async with self._engine.begin() as conn:
                ...
                if row is None:
                    res2 = await conn.execute(select(*category.c).where(category.c.name == name))
