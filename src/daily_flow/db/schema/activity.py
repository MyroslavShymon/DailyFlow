...
activity = Table(
    "activity",
    metadata,
    Column("id", Integer, primary_key=True, comment="Unique activity/idea entry identifier"),
    Column(
        "title", Text, nullable=False, comment="Short activity title (e.g., 'Walk in the park')."
    ),
    Column(
        "description",
        Text,
    ...
    # ---------- Energy / mood model ----------
    Column(
        "energy_required_min", Integer, nullable=True, comment="Min energy required to start (1–5)."
    ),
    Column(
        "energy_required_max",
        Integer,
    ...
    Column(
        "mood_min", Integer, nullable=True, comment="Min mood/state when this activity fits (1–5)."
    ),
    Column(
        "mood_max", Integer, nullable=True, comment="Max mood/state when this activity fits (1–5)."
    ),
    # ---------- Cost / prep / place ----------
    Column(
        "cost_level",
    ...
    CheckConstraint(
        "(people_count_min IS NULL OR people_count_max IS NULL) "
        "OR people_count_min <= people_count_max",
    ...
    CheckConstraint(
        "(duration_min_minutes IS NULL OR duration_max_minutes IS NULL) "
    ...
    CheckConstraint(
        "(energy_required_min IS NULL OR energy_required_max IS NULL) "
        "OR energy_required_min <= energy_required_max",
        name="ck_activity_energy_required_min_le_max",
    ),
    CheckConstraint(
        "energy_gain IS NULL OR (energy_gain BETWEEN 1 AND 5)", name="ck_activity_energy_gain_1_5"
    ),
    CheckConstraint(
        "mood_min IS NULL OR (mood_min BETWEEN 1 AND 5)", name="ck_activity_mood_min_1_5"
    ),
    CheckConstraint(
        "mood_max IS NULL OR (mood_max BETWEEN 1 AND 5)", name="ck_activity_mood_max_1_5"
    ),
    CheckConstraint(
    ...
    CheckConstraint("requires_preparation IN (0, 1)", name="ck_activity_requires_preparation_bool"),
    CheckConstraint(
...
activity_usage = Table(
    ...
    Column("used_at", DateTime, nullable=False, comment="When the activity happened (timestamp)."),
    Column(
    ...
    Column("rating_before", Integer, nullable=True, comment="Self-rating before (1–5)."),
    Column("rating_after", Integer, nullable=True, comment="Self-rating after (1–5)."),
    Column("mood_before", Integer, nullable=True, comment="Mood/state before (1–5)."),
    Column("mood_after", Integer, nullable=True, comment="Mood/state after (1–5)."),
    Column("energy_before", Integer, nullable=True, comment="Energy level before (1–5)."),
    Column("energy_after", Integer, nullable=True, comment="Energy level after (1–5)."),
    Column(
        "notes",
        Text,
        nullable=True,
    ...
    # Minimum consistency: if there is *_after, then
    # there should be used_at (already nullable=False) and preferably *_before
    CheckConstraint(
    ...
    comment="Activity usage log: when it happened, how long it took, and how state/ratings change",
)

category = Table(
    "category",
    metadata,
    Column("id", Integer, primary_key=True, comment="Unique category identifier."),
    Column(
        "name",
        Text,
        nullable=False,
