...
def check_columns_moods_mostly_missing(
    df: pd.DataFrame, contract: MoodLogIngestContract, threshold: float = 0.5
) -> CheckResult | None:
    ...
    mostly_empty_mask = pd.Series(
        df[mood_without_sleep].isna().mean(axis=1) > threshold, index=df.index
    )
    if mostly_empty_mask.any():
        ...
        return {"issue": issue, "mask": mostly_empty_mask}
