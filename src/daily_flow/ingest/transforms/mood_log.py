...
def normalize_mood_log(
    df_raws: list[dict[str, pd.DataFrame | str]], contract: MoodLogIngestContract
) -> pd.DataFrame:
    sheets = []

    for df_raw in df_raws:
        sheet = transform_mood_log_sheet(df_raw=df_raw["sheet"], contract=contract)
        sheets.append(sheet)
